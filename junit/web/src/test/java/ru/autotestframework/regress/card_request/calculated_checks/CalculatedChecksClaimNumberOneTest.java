...
@Tag("regress")
@Tag("card_request")
@Tag("calculated_checks_claim_№1")
@ClassName("На каждый кейс отдельная заявка. Карточка заявки. Рассчитанные проверки")
public class CalculatedChecksClaimNumberOneTest extends BaseTest {
    ...
    @Test
    @Tag("smoke")
    @Tag("check_block_3208031")
    @DisplayName("3208031 - Верификация. Просмотр блока  \"Верификация рассчитанные проверки\" отображение поля \"Причина назначения\"  ")
    @WorkItemIds({"3208031"})
    public void check_block_3208031(TestInfo testInfo) {
        claim = actionsClaimSteps.sendSclRequestToStandWithSpecifiedJson("data/json/claim_template_3862028.json", 1, testInfo).get(0);
        queuesPage
